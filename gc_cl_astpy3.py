...
if online == True:
    try:
        ...
        path = os.path.abspath(inspect.getfile(inspect.currentframe())).replace('gc_cl_astpy.py','gc_cat.fits')
        gc_cat = fits.open(path)[1].data

if online == False:
    path = os.path.abspath(inspect.getfile(inspect.currentframe())).replace('gc_cl_astpy3.py','gc_cat.txt')
    #gc_cat = fits.open(path)[1].data
